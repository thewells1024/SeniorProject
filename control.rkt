...
(: controlM (Unit (import language^ top^) (export language^)))
(define controlM
        ...
        (define (eval-lang exp context)
          (match exp
            [(list 'catch (? symbol? s) e)
             (Fx inVal (Continuation (λ ([v : Val]) : Computation
                         (match v
                           [(Continuation k)
                            (handler (eval-top e (extend-context context s v)) k)]
                           [_ (inExc 'not-continuation)]))))]
            [(list 'throw (? symbol? s) e)
             (handler (eval-top e context)
                      (λ ([v : Val]) : Computation
                        (match (ref-context context s)
                          [(Continuation cont) (Fx inVal (Continuation (λ (kp) (cont v))))]
                          [_ (inExc 'not-continuation)])))]
